...
class LogTest < MiniTest::Test
  def setup
    @log = Log.new("DietManager/.TXT's/diet_log.txt")
  end
  ...
  def test_add_log_item
    ...
    assert(@log.add_log_item(Date.today, "Bread Slice"), "LogItem not added to log")
    assert(@log.add_log_item(Date.new, "Jelly"), "LogItem not added to log")
    assert(@log.size == old_size + 2, "Size attribute not updated correctly")
  end
  ...
  def test_remove_log_item
    assert(@log.contains?("Apple", Date.parse("2008-09-12")), "LogItem that should exist not found")
    assert(@log.remove_log_item("Apple", Date.parse("2008-09-12")).name.eql?("Apple"), "Incorrect element removed from Log")
    assert(!@log.contains?("Apple", Date.parse("2008-09-12")), "LogItem that should not exist was mistakenly found")
    assert(@log.remove_log_item("Jelly", Date.today) == nil, "False positive, LogItem that does not exist was removed")
  end
end
